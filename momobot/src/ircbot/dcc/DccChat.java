...
public class DccChat implements IIrcSpecialChars {
     ...
     */
    public DccChat(final IrcUser user1, final long longAddress, final int port) {
        this.address = new InetSocketAddress(NetUtils.longToIp(longAddress),
                port);
     ...
     */
    public DccChat(final IrcUser user1, final Socket socket1)
            throws IOException {
        this.socket = socket1;
     ...
     *             en cas d'erreur
     */
    public final synchronized void accept() throws IOException {
        if (this.acceptable) {
            this.acceptable = false;
            this.socket = new Socket(this.address.getAddress(), this.address
                    .getPort());
            this.reader = new BufferedReader(new InputStreamReader(this.socket
                    .getInputStream()));
            this.writer = new PrintWriter(new BufferedWriter(
