...
public class Seek2Trigger extends AbstractTextTrigger implements IPrivmsgTrigger {
    ...
    public void doTrigger(final Privmsg im, final IIrcControl control) {
        ...
        int nbPlayers = Gather.DEFAULT_SIZE;
        String respMsg;
        try {
        ...
        SeekWar2 skwar = new SeekWar2(chan, control, server, nbPlayers, im.getMessage().substring(args[0].length() - 1,
                im.getMessage().length() - 1));
